...
def main():
    ...
    for i, k in enumerate(ks):
        for j, num in enumerate(num_anchs[i]):
            ...
            print('(k,num)=(%d,%d)' % (k, num), 'DP MAP: %.3f' % MAPs_dp[i,j], 'Fly MAP: %.3f' % MAPs_fly[i,j])

    np.savez('./experiment37_res.npz', MAPs_dp=MAPs_dp, MAPs_fly=MAPs_fly)
