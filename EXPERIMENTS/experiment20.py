import numpy as np
import matplotlib.pyplot as plt
import torch
from sklearn.neighbors import NearestNeighbors
import faiss

from data_classes import BERTDataset
from dist_perm import DistPerm

def main():

    n = 10000
    D = 128
    k = 128

    num_queries = 100
    R = 200
    d = 32
    #ks = [32, 64, 128, 256, 512, 1024]

    file_q = './datasets/Q.pt'
    file_db = './datasets/D.pt'
    data_source = BERTDataset(file_q, file_db, n)
    index_dp = DistPerm(k, dist='dot')

    index_l2 = faiss.IndexFlatL2(D)
    index_nn = NearestNeighbors(n_neighbors=R)

    db = data_source.generate_db()

        # index_lsh = faiss.IndexLSH(D, dj)
    index_pq = faiss.IndexPQ(D, d, 4)
    index_dp.d = d

    index_dp.fit(db)
    index_dp.add(db)
    data = np.array(db).astype(np.float32)
    index_pq.train(data)
    index_pq.add(data)
    index_l2.add(data)
    index_nn.fit(data)
        # index_l2.add(np.array(db))

    queries = data_source.generate_queries(num_queries)
    quers = np.array(queries).astype(np.float32)
    found = index_dp.search(queries, R).numpy()
    found2 = index_pq.search(quers, R)[1]
    true = index_l2.search(quers, R)[1]
    true2 = index_nn.kneighbors(quers, n_neighbors=R)[1]

    ap = num_queries*[0]
    ap2 = num_queries*[0]
    for i in range(num_queries):
        mismatch = np.argwhere(true[i] != true2[i]).squeeze()
        print(mismatch)
        hits = np.array([1*(f in true[i]) for f in found[i]])
        hits2 = np.array([1*(f in true2[i]) for f in found[i]])
        mismatch = np.argwhere(hits != hits2).squeeze()
        print(mismatch)
        pr = np.cumsum(hits)/np.arange(1, R+1)
        pr2 = np.cumsum(hits2)/np.arange(1, R+1)
        mismatch = np.argwhere(pr != pr2).squeeze()
        print(mismatch)
        ap[i] = sum(hits*pr)/R
        ap2[i] = sum(hits2*pr2)/R
        print(np.round(ap[i],3), np.round(ap2[i],3))
        
        print()

    print(np.mean(ap), np.mean(ap2))
        # precision_R = num_queries*[0]
        #avg_precision = num_queries*[0]
        #avg_precision2 = num_queries*[0]
        #ap = num_queries*[0]
        #ap2 = num_queries*[0]
        #for i in range(num_queries):
            #hits_vec = np.array([1*(f in true[i]) for f in found[i]])
            #hits_vec2 = np.array([1*(f in true[i]) for f in found2[i]])
            #hv = np.array([1*(f in true2[i]) for f in found[i]])
            #hv2 = np.array([1*(f in true2[i]) for f in found2[i]])

            #precision_R = np.cumsum(hits_vec)/np.arange(1, R+1)
            #precision_R2 = np.cumsum(hits_vec2)/np.arange(1, R+1)
            #pr = np.cumsum(hv)/np.arange(1, R+1)
            #pr2 = np.cumsum(hv2)/np.arange(1, R+1)

            #avg_precision[i] = sum(hits_vec*precision_R)/R
            #avg_precision2[i] = sum(hits_vec2*precision_R2)/R
            #ap[i] = sum(hv*pr)/R
            #ap2[i] = sum(hv2*pr2)/R

        #MAPs[j] = np.mean(avg_precision)
        #MAPs2[j] = np.mean(avg_precision2)
        #print('[1] AP@%d: %.3f +/- %.4f [DP + Faiss]' % (R, np.mean(avg_precision), np.std(avg_precision)))
        #print('[2] AP@%d: %.3f +/- %.4f [PQ + Faiss]' % (R, np.mean(avg_precision2), np.std(avg_precision2)))
        #print('[1] AP@%d: %.3f +/- %.4f [DP + kNN]' % (R, np.mean(ap), np.std(ap)))
        #print('[2] AP@%d: %.3f +/- %.4f [PQ + kNN]\n' % (R, np.mean(ap2), np.std(ap2)))

    #MAPs = np.array(MAPs)
    #MAPs2 = np.array(MAPs2)

    #plt.figure(figsize=(8,6))
    #plt.plot(ds, 100*MAPs, '--*k', label='Distance Permutation')
    #plt.plot(ds, 100*MAPs2, '--or', label='PQ')
    # plt.fill_between(Rs, 100*(means-2*stds), 100*(means+2*stds), color=(0.8, 0.8, 0.8))
    #plt.ylim([0, 100])
    #plt.ylabel('Mean Average Precision [%]')
    #plt.xlabel('Number of Anchors Used')
    #plt.title('Dataset with %d Entries, Retrieving Top-%s, # Anchors (Total) = %d' % (n, R, k))
    #plt.legend()

    #plt.savefig('./figures/experiment17.png', bbox_inches='tight')

    print('\nDONE\n')

if __name__ == '__main__':
    main()
